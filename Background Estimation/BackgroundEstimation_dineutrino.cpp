...
void BackgroundEstimation_dineutrino(){ 
  ...
  name.push_back("HT"); bin.push_back(30);     Min.push_back(0);    Max.push_back(3000);    axis.push_back("HT pt[GeV]");
  ...
  for(int i=0; i<name.size(); i++){
	...
	char CUTSR [1000];   char SR [1000];
	 
	sprintf(CUT,    "((category0==1)&&(Jet1ResolvedPt>0)&&(Jet2ResolvedPt>0)&&(Jet3ResolvedPt>0)&&(MinDeltaPhiJetMet>0.6)&&(TransverseMassMetTop>500)");

	sprintf(SR,     "(NumSelBJetsM>1)&&(NumSelLeps==0)&&(TopPt>340))"); 
	...
	sprintf(CS3,    "(NumSelBJetsM>1)&&(NumSelLeps==0)&&(TopPt>200)&&(TopPt<340))");   //ttbar:       signal selection with NumSelBJetsM>1 but TopPt<340
	//sprintf(CS3,    "(NumSelBJetsM>1)&&(NumSelLeps==0)&&(TopPt>340))");   //ttbar:       signal selection with NumSelBJetsM>1 but TopPt<340


	sprintf(CUTSR,     "PUWeight   *w_for  *w_Btag  *w_topJet  *w_WJet  *genWeight*%s&&%s",CUT,SR );
	...
	MakeHistos(CUTSR, CUTCS1,CUTCS2,CUTCS3,plot,bin[i],Min[i],Max[i],0,
...
void MakeHistos(char CUTSR[1000],char CUTCS1[1000],char CUTCS2[1000],char CUTCS3[1000],const char *plot,int BIN,float MIN,float MAX,int JETSyst,
  ...
  if(JETSyst==0){
    GetHisto(CUTSR, Tree01, data        ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree02, tptzm0700lh ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree04, tptzm0900lh ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree09, tptzm1400lh ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree12, tptzm1700lh ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree14, ZToNuNu_1   ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree15, ZToNuNu_2   ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree16, ZToNuNu_3   ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree17, ZToNuNu_4   ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree18, ZToNuNu_5   ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree19, ZToNuNu_6   ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree20, ZToNuNu_7   ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree21, QCD_1      ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree22, QCD_2      ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree23, QCD_3      ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree24, QCD_4      ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree25, QCD_5      ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree26, QCD_6      ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree27, QCD_7      ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree28, WToLNu_1   ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree29, WToLNu_2   ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree30, WToLNu_3   ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree31, WToLNu_4   ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree32, WToLNu_5   ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree33, WToLNu_6   ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree34, WToLNu_7   ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree35, TT_1    ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree36, TT_2    ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree37, ST_1    ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree38, ST_2    ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree39, ST_3    ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree40, ST_4    ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree41, ZZ1    ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree42, ZZ2    ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree43, ZZ3    ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree44, WW1    ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree45, WW2    ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree46, WZ1    ,plot,BIN,MIN,MAX);
    GetHisto(CUTSR, Tree47, WZ2    ,plot,BIN,MIN,MAX);
	GetHisto(CUTSR, Tree48, WZ3    ,plot,BIN,MIN,MAX);
  ...
  for(int j=1; j<data->GetNbinsX()+1; j++){
    ...
    float N_data_SR_err    = data->GetBinError(j)*data->GetBinError(j);
    float N_ZToNuNu_SR_err = ZToNuNu_1->GetBinError(j)*ZToNuNu_1->GetBinError(j) + ZToNuNu_2->GetBinError(j)*ZToNuNu_2->GetBinError(j) + ZToNuNu_3->GetBinError(j)*ZToNuNu_3->GetBinError(j) + ZToNuNu_4->GetBinError(j)*ZToNuNu_4->GetBinError(j) + ZToNuNu_5->GetBinError(j)*ZToNuNu_5->GetBinError(j) + ZToNuNu_6->GetBinError(j)*ZToNuNu_6->GetBinError(j) + ZToNuNu_7->GetBinError(j)*ZToNuNu_7->GetBinError(j);
    float N_QCD_SR_err     = QCD_1->GetBinError(j)*QCD_1->GetBinError(j) + QCD_2->GetBinError(j)*QCD_2->GetBinError(j) + QCD_3->GetBinError(j)*QCD_3->GetBinError(j) + QCD_4->GetBinError(j)*QCD_4->GetBinError(j) + QCD_5->GetBinError(j)*QCD_5->GetBinError(j) + QCD_6->GetBinError(j)*QCD_6->GetBinError(j) + QCD_7->GetBinError(j)*QCD_7->GetBinError(j);
    float N_WToLNu_SR_err  = WToLNu_1->GetBinError(j)*WToLNu_1->GetBinError(j)+WToLNu_2->GetBinError(j)*WToLNu_2->GetBinError(j)+WToLNu_3->GetBinError(j)*WToLNu_3->GetBinError(j)+WToLNu_4->GetBinError(j)*WToLNu_4->GetBinError(j)+WToLNu_5->GetBinError(j)*WToLNu_5->GetBinError(j)+WToLNu_6->GetBinError(j)*WToLNu_6->GetBinError(j)+WToLNu_7->GetBinError(j)*WToLNu_7->GetBinError(j);
	float N_TT_SR_err      = TT_1->GetBinError(j)*TT_1->GetBinError(j)+TT_2->GetBinError(j)*TT_2->GetBinError(j);
	float N_ST_SR_err      = ST_1->GetBinError(j)*ST_1->GetBinError(j) + ST_2->GetBinError(j)*ST_2->GetBinError(j)+ ST_3->GetBinError(j)*ST_3->GetBinError(j)+ ST_4->GetBinError(j)*ST_4->GetBinError(j);
    float N_VV_SR_err      = ZZ1->GetBinError(j)*ZZ1->GetBinError(j) + ZZ2->GetBinError(j)*ZZ2->GetBinError(j) + ZZ3->GetBinError(j)*ZZ3->GetBinError(j) + WW1->GetBinError(j)*WW1->GetBinError(j) + WW2->GetBinError(j)*WW2->GetBinError(j) + WZ1->GetBinError(j)*WZ1->GetBinError(j) + WZ2->GetBinError(j)*WZ2->GetBinError(j) + WZ3->GetBinError(j)*WZ3->GetBinError(j); 

	float N_data_CS1_err   = data_CS1->GetBinError(j)*data_CS1->GetBinError(j);
    float N_ZToNuNu_CS1_err= ZToNuNu_1_CS1->GetBinError(j)*ZToNuNu_1_CS1->GetBinError(j) + ZToNuNu_2_CS1->GetBinError(j)*ZToNuNu_2_CS1->GetBinError(j) + ZToNuNu_3_CS1->GetBinError(j)*ZToNuNu_3_CS1->GetBinError(j) + ZToNuNu_4_CS1->GetBinError(j)*ZToNuNu_4_CS1->GetBinError(j) + ZToNuNu_5_CS1->GetBinError(j)*ZToNuNu_5_CS1->GetBinError(j) + ZToNuNu_6_CS1->GetBinError(j)*ZToNuNu_6_CS1->GetBinError(j) + ZToNuNu_7_CS1->GetBinError(j)*ZToNuNu_7_CS1->GetBinError(j);
    float N_QCD_CS1_err    = QCD_1_CS1->GetBinError(j)*QCD_1_CS1->GetBinError(j) + QCD_2_CS1->GetBinError(j)*QCD_2_CS1->GetBinError(j) + QCD_3_CS1->GetBinError(j)*QCD_3_CS1->GetBinError(j) + QCD_4_CS1->GetBinError(j)*QCD_4_CS1->GetBinError(j) + QCD_5_CS1->GetBinError(j)*QCD_5_CS1->GetBinError(j) + QCD_6_CS1->GetBinError(j)*QCD_6_CS1->GetBinError(j) + QCD_7_CS1->GetBinError(j)*QCD_7_CS1->GetBinError(j);
	float N_WToLNu_CS1_err = WToLNu_1_CS1->GetBinError(j)*WToLNu_1_CS1->GetBinError(j)+WToLNu_2_CS1->GetBinError(j)*WToLNu_2_CS1->GetBinError(j)+WToLNu_3_CS1->GetBinError(j)*WToLNu_3_CS1->GetBinError(j)+WToLNu_4_CS1->GetBinError(j)*WToLNu_4_CS1->GetBinError(j)+WToLNu_5_CS1->GetBinError(j)*WToLNu_5_CS1->GetBinError(j)+WToLNu_6_CS1->GetBinError(j)*WToLNu_6_CS1->GetBinError(j)+WToLNu_7_CS1->GetBinError(j)*WToLNu_7_CS1->GetBinError(j);
	float N_TT_CS1_err     = TT_1_CS1->GetBinError(j)*TT_1_CS1->GetBinError(j)+TT_2_CS1->GetBinError(j)*TT_2_CS1->GetBinError(j);
	float N_ST_CS1_err     = ST_1_CS1->GetBinError(j)*ST_1_CS1->GetBinError(j) + ST_2_CS1->GetBinError(j)*ST_2_CS1->GetBinError(j)+ ST_3_CS1->GetBinError(j)*ST_3_CS1->GetBinError(j)+ ST_4_CS1->GetBinError(j)*ST_4_CS1->GetBinError(j);
    float N_VV_CS1_err     = ZZ1_CS1->GetBinError(j)*ZZ1_CS1->GetBinError(j) + ZZ2_CS1->GetBinError(j)*ZZ2_CS1->GetBinError(j) + ZZ3_CS1->GetBinError(j)*ZZ3_CS1->GetBinError(j) + WW1_CS1->GetBinError(j)*WW1_CS1->GetBinError(j) + WW2_CS1->GetBinError(j)*WW2_CS1->GetBinError(j) + WZ1_CS1->GetBinError(j)*WZ1_CS1->GetBinError(j) + WZ2_CS1->GetBinError(j)*WZ2_CS1->GetBinError(j) + WZ3_CS1->GetBinError(j)*WZ3_CS1->GetBinError(j); 
	

	float N_data_CS2_err   = data_CS2->GetBinError(j)*data_CS2->GetBinError(j);
    float N_ZToNuNu_CS2_err= ZToNuNu_1_CS2->GetBinError(j)*ZToNuNu_1_CS2->GetBinError(j) + ZToNuNu_2_CS2->GetBinError(j)*ZToNuNu_2_CS2->GetBinError(j) + ZToNuNu_3_CS2->GetBinError(j)*ZToNuNu_3_CS2->GetBinError(j) + ZToNuNu_4_CS2->GetBinError(j)*ZToNuNu_4_CS2->GetBinError(j) + ZToNuNu_5_CS2->GetBinError(j)*ZToNuNu_5_CS2->GetBinError(j) + ZToNuNu_6_CS2->GetBinError(j)*ZToNuNu_6_CS2->GetBinError(j) + ZToNuNu_7_CS2->GetBinError(j)*ZToNuNu_7_CS2->GetBinError(j);
    float N_QCD_CS2_err    = QCD_1_CS2->GetBinError(j)*QCD_1_CS2->GetBinError(j) + QCD_2_CS2->GetBinError(j)*QCD_2_CS2->GetBinError(j) + QCD_3_CS2->GetBinError(j)*QCD_3_CS2->GetBinError(j) + QCD_4_CS2->GetBinError(j)*QCD_4_CS2->GetBinError(j) + QCD_5_CS2->GetBinError(j)*QCD_5_CS2->GetBinError(j) + QCD_6_CS2->GetBinError(j)*QCD_6_CS2->GetBinError(j) + QCD_7_CS2->GetBinError(j)*QCD_7_CS2->GetBinError(j);
	float N_WToLNu_CS2_err = WToLNu_1_CS2->GetBinError(j)*WToLNu_1_CS2->GetBinError(j)+WToLNu_2_CS2->GetBinError(j)*WToLNu_2_CS2->GetBinError(j)+WToLNu_3_CS2->GetBinError(j)*WToLNu_3_CS2->GetBinError(j)+WToLNu_4_CS2->GetBinError(j)*WToLNu_4_CS2->GetBinError(j)+WToLNu_5_CS2->GetBinError(j)*WToLNu_5_CS2->GetBinError(j)+WToLNu_6_CS2->GetBinError(j)*WToLNu_6_CS2->GetBinError(j)+WToLNu_7_CS2->GetBinError(j)*WToLNu_7_CS2->GetBinError(j);
	float N_TT_CS2_err     = TT_1_CS2->GetBinError(j)*TT_1_CS2->GetBinError(j)+TT_2_CS2->GetBinError(j)*TT_2_CS2->GetBinError(j);
	float N_ST_CS2_err     = ST_1_CS2->GetBinError(j)*ST_1_CS2->GetBinError(j) + ST_2_CS2->GetBinError(j)*ST_2_CS2->GetBinError(j)+ ST_3_CS2->GetBinError(j)*ST_3_CS2->GetBinError(j)+ ST_4_CS2->GetBinError(j)*ST_4_CS2->GetBinError(j);
    float N_VV_CS2_err     = ZZ1_CS2->GetBinError(j)*ZZ1_CS2->GetBinError(j) + ZZ2_CS2->GetBinError(j)*ZZ2_CS2->GetBinError(j) + ZZ3_CS2->GetBinError(j)*ZZ3_CS2->GetBinError(j) + WW1_CS2->GetBinError(j)*WW1_CS2->GetBinError(j) + WW2_CS2->GetBinError(j)*WW2_CS2->GetBinError(j) + WZ1_CS2->GetBinError(j)*WZ1_CS2->GetBinError(j) + WZ2_CS2->GetBinError(j)*WZ2_CS2->GetBinError(j) + WZ3_CS2->GetBinError(j)*WZ3_CS2->GetBinError(j);  
    

	float N_data_CS3_err   = data_CS3->GetBinError(j)*data_CS3->GetBinError(j);
    float N_ZToNuNu_CS3_err= ZToNuNu_1_CS3->GetBinError(j)*ZToNuNu_1_CS3->GetBinError(j) + ZToNuNu_2_CS3->GetBinError(j)*ZToNuNu_2_CS3->GetBinError(j) + ZToNuNu_3_CS3->GetBinError(j)*ZToNuNu_3_CS3->GetBinError(j) + ZToNuNu_4_CS3->GetBinError(j)*ZToNuNu_4_CS3->GetBinError(j) + ZToNuNu_5_CS3->GetBinError(j)*ZToNuNu_5_CS3->GetBinError(j) + ZToNuNu_6_CS3->GetBinError(j)*ZToNuNu_6_CS3->GetBinError(j) + ZToNuNu_7_CS3->GetBinError(j)*ZToNuNu_7_CS3->GetBinError(j);
    float N_QCD_CS3_err    = QCD_1_CS3->GetBinError(j)*QCD_1_CS3->GetBinError(j) + QCD_2_CS3->GetBinError(j)*QCD_2_CS3->GetBinError(j) + QCD_3_CS3->GetBinError(j)*QCD_3_CS3->GetBinError(j) + QCD_4_CS3->GetBinError(j)*QCD_4_CS3->GetBinError(j) + QCD_5_CS3->GetBinError(j)*QCD_5_CS3->GetBinError(j) + QCD_6_CS3->GetBinError(j)*QCD_6_CS3->GetBinError(j) + QCD_7_CS3->GetBinError(j)*QCD_7_CS3->GetBinError(j);
	float N_WToLNu_CS3_err = WToLNu_1_CS3->GetBinError(j)*WToLNu_1_CS3->GetBinError(j)+WToLNu_2_CS3->GetBinError(j)*WToLNu_2_CS3->GetBinError(j)+WToLNu_3_CS3->GetBinError(j)*WToLNu_3_CS3->GetBinError(j)+WToLNu_4_CS3->GetBinError(j)*WToLNu_4_CS3->GetBinError(j)+WToLNu_5_CS3->GetBinError(j)*WToLNu_5_CS3->GetBinError(j)+WToLNu_6_CS3->GetBinError(j)*WToLNu_6_CS3->GetBinError(j)+WToLNu_7_CS3->GetBinError(j)*WToLNu_7_CS3->GetBinError(j);
	float N_TT_CS3_err     = TT_1_CS3->GetBinError(j)*TT_1_CS3->GetBinError(j)+TT_2_CS3->GetBinError(j)*TT_2_CS3->GetBinError(j);
	float N_ST_CS3_err     = ST_1_CS3->GetBinError(j)*ST_1_CS3->GetBinError(j) + ST_2_CS3->GetBinError(j)*ST_2_CS3->GetBinError(j)+ ST_3_CS3->GetBinError(j)*ST_3_CS3->GetBinError(j)+ ST_4_CS3->GetBinError(j)*ST_4_CS3->GetBinError(j);
    float N_VV_CS3_err     = ZZ1_CS3->GetBinError(j)*ZZ1_CS3->GetBinError(j) + ZZ2_CS3->GetBinError(j)*ZZ2_CS3->GetBinError(j) + ZZ3_CS3->GetBinError(j)*ZZ3_CS3->GetBinError(j) + WW1_CS3->GetBinError(j)*WW1_CS3->GetBinError(j) + WW2_CS3->GetBinError(j)*WW2_CS3->GetBinError(j) + WZ1_CS3->GetBinError(j)*WZ1_CS3->GetBinError(j) + WZ2_CS3->GetBinError(j)*WZ2_CS3->GetBinError(j) + WZ3_CS3->GetBinError(j)*WZ3_CS3->GetBinError(j); 
    ...
    float N_data_SR    = data->GetBinContent(j);
    float N_ZToNuNu_SR = ZToNuNu_1->GetBinContent(j) + ZToNuNu_2->GetBinContent(j) + ZToNuNu_3->GetBinContent(j) + ZToNuNu_4->GetBinContent(j) + ZToNuNu_5->GetBinContent(j) + ZToNuNu_6->GetBinContent(j) + ZToNuNu_7->GetBinContent(j);
    float N_QCD_SR     = QCD_1->GetBinContent(j) + QCD_2->GetBinContent(j) + QCD_3->GetBinContent(j) + QCD_4->GetBinContent(j) + QCD_5->GetBinContent(j) + QCD_6->GetBinContent(j) + QCD_7->GetBinContent(j);
    float N_WToLNu_SR  = WToLNu_1->GetBinContent(j) + WToLNu_2->GetBinContent(j) + WToLNu_3->GetBinContent(j) + WToLNu_4->GetBinContent(j) + WToLNu_5->GetBinContent(j) + WToLNu_6->GetBinContent(j)+ WToLNu_7->GetBinContent(j);
	float N_TT_SR      = TT_1->GetBinContent(j) + TT_2->GetBinContent(j);
    float N_ST_SR      = ST_1 ->GetBinContent(j) + ST_2 ->GetBinContent(j) + ST_3 ->GetBinContent(j) + ST_4 ->GetBinContent(j);
	float N_VV_SR      = ZZ1->GetBinContent(j)+ZZ2->GetBinContent(j)+ZZ3->GetBinContent(j)+WW1->GetBinContent(j)+WW2->GetBinContent(j)+WZ1->GetBinContent(j)+WZ2->GetBinContent(j)+WZ3->GetBinContent(j);

	float N_data_CS1   = data_CS1->GetBinContent(j);
    float N_ZToNuNu_CS1= ZToNuNu_1_CS1->GetBinContent(j) + ZToNuNu_2_CS1->GetBinContent(j) + ZToNuNu_3_CS1->GetBinContent(j) + ZToNuNu_4_CS1->GetBinContent(j) + ZToNuNu_5_CS1->GetBinContent(j) + ZToNuNu_6_CS1->GetBinContent(j) + ZToNuNu_7_CS1->GetBinContent(j);
    float N_QCD_CS1    = QCD_1_CS1->GetBinContent(j) + QCD_2_CS1->GetBinContent(j) + QCD_3_CS1->GetBinContent(j) + QCD_4_CS1->GetBinContent(j) + QCD_5_CS1->GetBinContent(j) + QCD_6_CS1->GetBinContent(j) + QCD_7_CS1->GetBinContent(j);
	float N_WToLNu_CS1 = WToLNu_1_CS1->GetBinContent(j) + WToLNu_2_CS1->GetBinContent(j) + WToLNu_3_CS1->GetBinContent(j) + WToLNu_4_CS1->GetBinContent(j) + WToLNu_5_CS1->GetBinContent(j) + WToLNu_6_CS1->GetBinContent(j)+ WToLNu_7_CS1->GetBinContent(j);
	float N_TT_CS1     = TT_1_CS1->GetBinContent(j) + TT_2_CS1->GetBinContent(j);
    float N_ST_CS1     = ST_1_CS1->GetBinContent(j) + ST_2_CS1->GetBinContent(j) + ST_3_CS1 ->GetBinContent(j) + ST_4_CS1 ->GetBinContent(j);
	float N_VV_CS1     = ZZ1_CS1->GetBinContent(j)+ZZ2_CS1->GetBinContent(j)+ZZ3_CS1->GetBinContent(j)+WW1_CS1->GetBinContent(j)+WW2_CS1->GetBinContent(j)+WZ1_CS1->GetBinContent(j)+WZ2_CS1->GetBinContent(j)+WZ3_CS1->GetBinContent(j);

	
	float N_data_CS2   = data_CS2->GetBinContent(j);
    float N_ZToNuNu_CS2= ZToNuNu_1_CS2->GetBinContent(j) + ZToNuNu_2_CS2->GetBinContent(j) + ZToNuNu_3_CS2->GetBinContent(j) + ZToNuNu_4_CS2->GetBinContent(j) + ZToNuNu_5_CS2->GetBinContent(j) + ZToNuNu_6_CS2->GetBinContent(j) + ZToNuNu_7_CS2->GetBinContent(j);
    float N_QCD_CS2    = QCD_1_CS2->GetBinContent(j) + QCD_2_CS2->GetBinContent(j) + QCD_3_CS2->GetBinContent(j) + QCD_4_CS2->GetBinContent(j) + QCD_5_CS2->GetBinContent(j) + QCD_6_CS2->GetBinContent(j) + QCD_7_CS2->GetBinContent(j);
	float N_WToLNu_CS2 = WToLNu_1_CS2->GetBinContent(j) + WToLNu_2_CS2->GetBinContent(j) + WToLNu_3_CS2->GetBinContent(j) + WToLNu_4_CS2->GetBinContent(j) + WToLNu_5_CS2->GetBinContent(j) + WToLNu_6_CS2->GetBinContent(j)+ WToLNu_7_CS2->GetBinContent(j);
	float N_TT_CS2     = TT_1_CS2->GetBinContent(j) + TT_2_CS2->GetBinContent(j);
    float N_ST_CS2     = ST_1_CS2->GetBinContent(j) + ST_2_CS2->GetBinContent(j) + ST_3_CS2 ->GetBinContent(j) + ST_4_CS2 ->GetBinContent(j);
	float N_VV_CS2     = ZZ1_CS2->GetBinContent(j)+ZZ2_CS2->GetBinContent(j)+ZZ3_CS2->GetBinContent(j)+WW1_CS2->GetBinContent(j)+WW2_CS2->GetBinContent(j)+WZ1_CS2->GetBinContent(j)+WZ2_CS2->GetBinContent(j)+WZ3_CS2->GetBinContent(j);


	float N_data_CS3   = data_CS3->GetBinContent(j);
    float N_ZToNuNu_CS3= ZToNuNu_1_CS3->GetBinContent(j) + ZToNuNu_2_CS3->GetBinContent(j) + ZToNuNu_3_CS3->GetBinContent(j) + ZToNuNu_4_CS3->GetBinContent(j) + ZToNuNu_5_CS3->GetBinContent(j) + ZToNuNu_6_CS3->GetBinContent(j) + ZToNuNu_7_CS3->GetBinContent(j);
    float N_QCD_CS3    = QCD_1_CS3->GetBinContent(j) + QCD_2_CS3->GetBinContent(j) + QCD_3_CS3->GetBinContent(j) + QCD_4_CS3->GetBinContent(j) + QCD_5_CS3->GetBinContent(j) + QCD_6_CS3->GetBinContent(j) + QCD_7_CS3->GetBinContent(j);
	float N_WToLNu_CS3 = WToLNu_1_CS3->GetBinContent(j) + WToLNu_2_CS3->GetBinContent(j) + WToLNu_3_CS3->GetBinContent(j) + WToLNu_4_CS3->GetBinContent(j) + WToLNu_5_CS3->GetBinContent(j) + WToLNu_6_CS3->GetBinContent(j)+ WToLNu_7_CS3->GetBinContent(j);
	float N_TT_CS3     = TT_1_CS3->GetBinContent(j) + TT_2_CS3->GetBinContent(j);
    float N_ST_CS3     = ST_1_CS3->GetBinContent(j) + ST_2_CS3->GetBinContent(j) + ST_3_CS3 ->GetBinContent(j) + ST_4_CS3 ->GetBinContent(j);
	float N_VV_CS3     = ZZ1_CS3->GetBinContent(j)+ZZ2_CS3->GetBinContent(j)+ZZ3_CS3->GetBinContent(j)+WW1_CS3->GetBinContent(j)+WW2_CS3->GetBinContent(j)+WZ1_CS3->GetBinContent(j)+WZ2_CS3->GetBinContent(j)+WZ3_CS3->GetBinContent(j);
	...
	float a1 = N_ZToNuNu_CS1;  float b1 = N_WToLNu_CS1;  float c1 = N_TT_CS1;  float d1 = N_QCD_CS1 + N_ST_CS1 + N_VV_CS1 - N_data_CS1;
	float a2 = N_ZToNuNu_CS2;  float b2 = N_WToLNu_CS2;  float c2 = N_TT_CS2;  float d2 = N_QCD_CS2 + N_ST_CS2 + N_VV_CS2 - N_data_CS2;
	float a3 = N_ZToNuNu_CS3;  float b3 = N_WToLNu_CS3;  float c3 = N_TT_CS3;  float d3 = N_QCD_CS3 + N_ST_CS3 + N_VV_CS3 - N_data_CS3;

	float a1_err = N_ZToNuNu_CS1_err;  float b1_err = N_WToLNu_CS1_err;  float c1_err = N_TT_CS1_err;  float d1_err = N_QCD_CS1_err + N_ST_CS1_err + N_VV_CS1_err ;
	float a2_err = N_ZToNuNu_CS2_err;  float b2_err = N_WToLNu_CS2_err;  float c2_err = N_TT_CS2_err;  float d2_err = N_QCD_CS2_err + N_ST_CS2_err + N_VV_CS2_err ;
	float a3_err = N_ZToNuNu_CS3_err;  float b3_err = N_WToLNu_CS3_err;  float c3_err = N_TT_CS3_err;  float d3_err = N_QCD_CS3_err + N_ST_CS3_err + N_VV_CS3_err ;

	float SF_den  = a3*b2*c1 - a2*b3*c1 - a3*b1*c2 + a1*b3*c2 + a2*b1*c3 - a1*b2*c3;
	float SF1_num = b3*c2*d1 - b2*c3*d1 - b3*c1*d2 + b1*c3*d2 + b2*c1*d3 - b1*c2*d3;
	float SF2_num =-a3*c2*d1 + a2*c3*d1 + a3*c1*d2 - a1*c3*d2 - a2*c1*d3 + a1*c2*d3;
	float SF3_num = a3*b2*d1 - a2*b3*d1 - a3*b1*d2 + a1*b3*d2 + a2*b1*d3 - a1*b2*d3;

	if (SF_den==0) continue;

  float SF1 = -SF1_num/SF_den;
	float SF2 = -SF1_num/SF_den;
	float SF3 = -SF3_num/SF_den;
  ...
  SF3_num_err =(a3_err*b2*d1*b2*d1 + b2_err*a3*d1*a3*d1 + d1_err*a3*b2*a3*b2) + 
	...
	float SF3_err = (SF3_num_err*SF_den*SF_den + SF_den_err*SF3_num*SF3_num)/(SF_den*SF_den*SF_den*SF_den);
  ...
  */
  ...
  delete data; delete background; delete background_PRE;
  ...
  delete data_CS1; delete background_CS1; 
  ...
  delete data_CS2; delete background_CS2; 
  ...
  delete data_CS3; delete background_CS3; 
