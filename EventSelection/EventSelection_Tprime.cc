...
void EventSelection_Tprime_0700(){
  ...
  for(int Nfiles=0; Nfiles<fileName.size(); Nfiles++){
    ...
    //string FILEprov = "root://eosuser.cern.ch:1094//eos/user/a/aspiezia/TPrime/2016/v08/"+fileName[Nfiles];
	...
	initializeVar();
	if( electrons) {if(!(HLT_Ele115_CaloIdVT_GsfTrkIdT_==1))    continue;}
	if(!electrons) {if(!(HLT_IsoMu24_==1 || HLT_IsoTkMu24_==1)) continue;}
...
void get_weight_btag(int selection, float &w_Btag, float &w_BtagUp, float &w_BtagDown, float &w_BtagLoose, float &w_BtagLooseUp, float &w_BtagLooseDown, string fileName){
  string FILEprov = "SF/BtagEfficiency/"+fileName;
  const char *FILE = FILEprov.c_str();
...
void ElectronTriggerSF(float pt, float eta){
  ...
  w_TrigElec    = histoElectronTrig->GetBinContent(X1,Y1);
  w_TrigElecUp  = histoElectronTrig->GetBinContent(X1,Y1) + histoElectronTrig->GetBinError(X1,Y1);
  w_TrigElecDown= histoElectronTrig->GetBinContent(X1,Y1) - histoElectronTrig->GetBinError(X1,Y1);
