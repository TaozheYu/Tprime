#include "EventSelection_dineutrino_2016.h" 
#include "math.h"

void EventSelection_dineutrino_2016(const char * Input = ""){
  ...
  for(unsigned int Nfiles=0; Nfiles<fileName.size(); Nfiles++){
    string NewFileprov;
    if ((SysJes==0)&&(SysJer==0)) NewFileprov = "/publicfs/cms/user/yutz/Tprime/2016_dineutrino/Preselection/"+fileName[Nfiles];
	if ((SysJes==1)&&(SysJer==0)) NewFileprov = "/publicfs/cms/user/yutz/Tprime/2016_dineutrino/Preselection/JESup/"+fileName[Nfiles];
	if ((SysJes==2)&&(SysJer==0)) NewFileprov = "/publicfs/cms/user/yutz/Tprime/2016_dineutrino/Preselection/JESdo/"+fileName[Nfiles];
	if ((SysJes==0)&&(SysJer==1)) NewFileprov = "/publicfs/cms/user/yutz/Tprime/2016_dineutrino/Preselection/JERup/"+fileName[Nfiles];
	if ((SysJes==0)&&(SysJer==2)) NewFileprov = "/publicfs/cms/user/yutz/Tprime/2016_dineutrino/Preselection/JERdo/"+fileName[Nfiles];
    ...
    string FILEprov = "/publicfs/cms/user/yutz/Tprime/2016_dineutrino/data_and_sample_2016/"+fileName[Nfiles];
	...
	initializeVar();
	if(!(HLT_PFMETNoMu120_PFMHTNoMu120_IDTight_==1 || HLT_PFMET120_PFMHT120_IDTight_==1))  continue;
	...
	if(!data){
	  ...
	  if(!(fileName[Nfiles].find("ST_tW")!=string::npos )) fillgenWeights();
	}
...
void SelectJets(int jetType, vector<TLorentzVector> & SelectedJets, vector<float> & SelectedJetsCSV, vector<TLorentzVector> SelectedElectrons, vector<TLorentzVector> SelectedMuons, int SysJes, int SysJer, bool data, bool &deltaPhiJetMet){
  ...
  for (UInt_t j = 0; j < Jet_pt_->size(); ++j){
	...
	if (jetType!=2) {if(!(jetpt>30))                                        continue;}
...
void get_weight_btag(int selection, float &w_Btag, float &w_BtagUp, float &w_BtagDown, float &w_Btag1Up, float &w_Btag1Down, float &w_Btag2Up, float &w_Btag2Down, float &w_BtagLoose, float &w_BtagLooseUp, float &w_BtagLooseDown, string fileName){
  string FILEprov = "/publicfs/cms/user/yutz/Tprime/2016_dineutrino/BtagEfficiency/"+fileName;
  const char *FILE = FILEprov.c_str();
