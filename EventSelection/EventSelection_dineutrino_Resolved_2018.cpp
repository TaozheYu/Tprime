#include "EventSelection_dineutrino_Resolved_2018.h" 
#include "math.h"

void EventSelection_dineutrino_Resolved_2018(const char * Input = ""){
  ...
  for(unsigned int Nfiles=0; Nfiles<fileName.size(); Nfiles++){
    ...
    string FILEprov = "/publicfs/cms/user/yutz/Tprime/2018_dineutrino/data_and_sample_v2/"+fileName[Nfiles];
    const char *FILE = FILEprov.c_str();
    TFile *file = TFile::Open(FILE);
    Tree = (TTree*)file->Get(openTree);
    bool data = true;
    if(!(fileName[Nfiles].find("Run")!=string::npos )) data = false;
	...
	bool SelectedMet = false;
        ...
        if( Met_pt > 200)  SelectedMet = true;
	...
	EVENT_genHT       = EVENT_genHT_;
        ...
        MHT               = MHTcalculator(SelectedJets);
	...
	if(!data){
	  ...
	  GenWeight(fileName[Nfiles], GenZPt, GenWPt);
          HTSF(fileName[Nfiles],HT,Met_pt,w_ZToNuNu,w_ZToNuNuUp,w_ZToNuNuDown,w_WToLNu,w_WToLNuUp,w_WToLNuDown,w_ttbar,w_ttbarUp,w_ttbarDown);
...
void MetCorrection(int SysJes, int SysJer, float &MET){
     float METx = Met_type1PF_pt_*TMath::Cos(Met_type1PF_phi_);
...
void SelectElectrons(vector<TLorentzVector> & SelectedElectrons, vector<int> & SelectedElectronsIndex, bool data){
  for (UInt_t j = 0; j < patElectron_pt_->size(); ++j){
    ...
    if(!(fabs(patElectron_eta_->at(j))<2.6))	     continue;
...
void SelectMuons(vector<TLorentzVector> & SelectedMuons, vector<int> & SelectedMuonsIndex){
  for (UInt_t j = 0; j < Muon_pt_->size(); ++j){
    ...
    if(!(fabs(Muon_eta_->at(j))<2.6))             continue;
...
void SelectJets(int jetType, vector<TLorentzVector> & SelectedJets, vector<float> & SelectedJetsCSV, vector<TLorentzVector> SelectedElectrons, vector<TLorentzVector> SelectedMuons, int SysJes, int SysJer, bool data, bool &deltaPhiJetMet, bool &HEMveto, float &w_HEM){
  ...
  for (UInt_t j = 0; j < Jet_pt_->size(); ++j){
    ...
    } else if(fabs(Jet_eta_->at(j))>2.6 && fabs(Jet_eta_->at(j))<2.7){
      if(!(Jet_neutralHadEnergyFraction_->at(j)<0.90))                      continue;
      if(!(Jet_neutralEmEnergyFraction_->at(j)<0.90))                       continue;
      if(!(Jet_numberOfConstituents_->at(j)>1))                             continue;
    ...
    if(jetType==11){if(!(Jet_pfDeepCSVBJetTags_->at(j)>0.1241)) continue;}
...
void SelectCA8Jets(int CA8jetType,vector<TLorentzVector> & SelectedCA8Jets,vector<TLorentzVector> SelectedElectrons,vector<TLorentzVector> SelectedMuons, vector<int> & CA8Indices,int SysJes, int SysJer, bool data, bool &deltaPhiJetMet){
  //CA8jetType=0 -> W-jets
  ...
  for (UInt_t j = 0; j < BoostedJet_pt_->size(); ++j){
    ...
    if(CA8jetType==0){
      ...
      if(!(BoostedJet_puppi_tau2_->at(j)/BoostedJet_puppi_tau1_->at(j)+0.08*log(BoostedJet_puppi_softdrop_mass_->at(j)*BoostedJet_puppi_softdrop_mass_->at(j)/(jetpt))<0.45)) continue;
  ...
  for(unsigned int i=0; i<SelectedJets.size(); i++){
    for(unsigned int j=i+1; j<SelectedJets.size(); j++){
      for(unsigned int k=j+1; k<SelectedJets.size(); k++){
         ...
         //if(!(fabs((SelectedJets[i]+SelectedJets[j]+SelectedJets[k]).M()-173.1)<TopMassInitial)) continue;
         if(TopMassCut){if(!((SelectedJets[i]+SelectedJets[j]+SelectedJets[k]).M()>100 && (SelectedJets[i]+SelectedJets[j]+SelectedJets[k]).M()<300)) continue;}
         if(btag){if(!(SelectedJetsCSV[i]>0.4184 || SelectedJetsCSV[j]>0.4184 || SelectedJetsCSV[k]>0.4184)) continue;}
...
void PartiallyMergedSelection(bool &PartiallyMerged,vector<TLorentzVector> SelectedWJets,vector<TLorentzVector> SelectedJets,vector<float> SelectedJetsCSV,TLorentzVector &TopQuark,TLorentzVector &Jet1,TLorentzVector &Jet2,bool TopMassCut,bool btag,vector<int> CA8Indices,int & CA8Index,float &WMass_,float &WSubjet_, bool data){
  ...
  for(unsigned int i=0; i<SelectedWJets.size(); i++){
    for(unsigned int k=0; k<SelectedJets.size(); k++){
      ...
      if(btag){if(!(SelectedJetsCSV[k]>0.4184))                                                                       continue;}
...
void get_weight_btag(int selection, float &w_Btag, float &w_BtagUp, float &w_BtagDown, float &w_Btag1Up, float &w_Btag1Down, float &w_Btag2Up, float &w_Btag2Down, float &w_BtagLoose, float &w_BtagLooseUp, float &w_BtagLooseDown, string fileName){
  string FILEprov = "/publicfs/cms/user/yutz/Tprime/2018_dineutrino/BtagEfficiency/"+fileName;
  const char *FILE = FILEprov.c_str();
  ...
  for (UInt_t j = 0; j < Jet_pt_->size(); ++j){
    ...
    if(!(fabs(Jet_eta_->at(j))<2.6))                                      continue;
    if(!(Jet_neutralHadEnergyFraction_->at(j)<0.90))                      continue;
    if(!(Jet_neutralEmEnergyFraction_->at(j)<0.90))                       continue;
    if(!(Jet_numberOfConstituents_->at(j)>1))                             continue;
    if(!(Jet_chargedHadronEnergyFraction_->at(j)>0.0))                    continue;
    ...
    if(Jet_pfDeepCSVBJetTags_->at(j)>0.4184){
      mcTagMedium *= eff;
    ...
    if(Jet_pfDeepCSVBJetTags_->at(j)>0.1241){
      mcTagLoose *= eff;
      dataTagLoose *= eff*SF;
...
void HTSF(string fileName, float HT, float Met_pt, float &w_SF1, float &w_SF1Up, float &w_SF1Down, float &w_SF2, float &w_SF2Up, float &w_SF2Down, float &w_SF3, float &w_SF3Up, float &w_SF3Down){
  if(fileName.find("ZToNuNu")!=string::npos ){
        ...
        if (htBin==0 || metBin==0){
           ...
           w_SF1Up = histoSF1->GetBinContent(htBin,metBin)+ histoSF1->GetBinError(htBin,metBin);
