...
void plottingTrigger_dineutrino(){
  ...
  using namespace std;
  char openTree[500];   sprintf(openTree, "BOOM");                                                                        //1.定义要打开的tree文件 
  TFile *file1 = TFile::Open("root://eosuser.cern.ch:1094//eos/user/a/aspiezia/TPrime/2017/TRIGGER/Tprime_0800.root");    //2.定义要打开的.root文件
  ...
  for(int Nfiles=1; Nfiles<3; Nfiles++){
    if(Nfiles==0) Tree = (TTree*)file1->Get(openTree);                                                                             //3.打开.root文件中的tree 
    if(Nfiles==1) Tree = (TTree*)file2->Get(openTree); 
    if(Nfiles==2) Tree = (TTree*)file3->Get(openTree);  
    branch();                                                                                                                      //4.为tree中的branch(变量)设置地址
...
void HistoFill(int Nfiles){
  if(Nfiles==0){
	 ...
	 h_MET_pt_DEN_0800->Fill(MET_pt_DEN);
	 h_MET_pt_NUM_PassTrigger_0800->Fill(MET_pt_NUM_PassTrigger);
...
void writeFile(){
  ...
  h_MET_pt_DEN_0800->Write();
  h_MET_pt_NUM_PassTrigger_0800->Write();
